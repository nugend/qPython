...
EXPRESSIONS = OrderedDict(
    (
        ...
        (
            ...
            qlist(
                numpy.array(
                    [numpy.string_("the"), numpy.string_("quick"), numpy.string_("brown"), numpy.string_("fox")], dtype=numpy.object
                ),
                qtype=QSYMBOL_LIST,
            ),
        ),
        ...
        (
            ...
            QDictionary(
                ...
                [
                    ...
                    QDictionary(qlist(numpy.array(["x", "y"]), qtype=QSYMBOL_LIST), [b"a", numpy.int64(2)]),
                    numpy.float32(5.5),
                ],
            ),
        ),
        ...
        (
            ...
            qtable(
                qlist(numpy.array(["name", "iq"]), qtype=QSYMBOL_LIST),
                [qlist(numpy.array([], dtype=numpy.string_), qtype=QSYMBOL_LIST), qlist(numpy.array([]), qtype=QINT_LIST)],
            ),
        ),
        (
...
def compare(left, right):
    ...
    if type(left) == QTemporal and numpy.isnat(left.raw):
        return numpy.isnat(right.raw)
    elif type(left) == numpy.datetime64 and numpy.isnat(left):
        return numpy.isnat(right)
    elif type(left) == numpy.timedelta64 and numpy.isnat(left):
        return numpy.isnat(right)
