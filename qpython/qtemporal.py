...
def _to_qmonth(dt):
    ...
    if t_dt == numpy.int32:
        ...
        return (dt - _EPOCH_QMONTH).astype(int) if not numpy.isnat(dt) else _QMONTH_NULL
    else:
...
def _to_qdate(dt):
    ...
    if t_dt == numpy.int32:
        ...
        return (dt - _EPOCH_QDATE).astype(int) if not numpy.isnat(dt) else _QDATE_NULL
    else:
        raise ValueError('Cannot convert %s of type %s to q value.' % (dt, type(dt)))
...
def _to_qdatetime(dt):
    ...
    if t_dt == numpy.float64:
        ...
        return (dt - _EPOCH_QDATETIME).astype(float) / _MILLIS_PER_DAY if not numpy.isnat(dt) else _QDATETIME_NULL
    else:
        raise ValueError('Cannot convert %s of type %s to q value.' % (dt, type(dt)))
...
def _to_qminute(dt):
    ...
    if t_dt == numpy.int32:
        ...
        return dt.astype(int) if not numpy.isnat(dt) else _QMINUTE_NULL
    else:
        raise ValueError('Cannot convert %s of type %s to q value.' % (dt, type(dt)))
...
def _to_qsecond(dt):
    ...
    if t_dt == numpy.int32:
        ...
        return dt.astype(int) if not numpy.isnat(dt) else _QSECOND_NULL
    else:
        raise ValueError('Cannot convert %s of type %s to q value.' % (dt, type(dt)))
...
def _to_qtime(dt):
    ...
    if t_dt == numpy.int32:
        ...
        return dt.astype(int) if not numpy.isnat(dt) else _QTIME_NULL
    else:
        raise ValueError('Cannot convert %s of type %s to q value.' % (dt, type(dt)))
...
def _to_qtimestamp(dt):
    ...
    if t_dt == numpy.int64:
        ...
        return (dt - _EPOCH_TIMESTAMP).astype(longlong) if not numpy.isnat(dt) else _QTIMESTAMP_NULL
    else:
        raise ValueError('Cannot convert %s of type %s to q value.' % (dt, type(dt)))
...
def _to_qtimespan(dt):
    ...
    if t_dt == numpy.int64:
        ...
        return dt.astype(longlong) if not numpy.isnat(dt) else _QTIMESTAMP_NULL
    else:
        raise ValueError('Cannot convert %s of type %s to q value.' % (dt, type(dt)))
